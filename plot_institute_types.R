pacman::p_load(ggplot2, tidyverse, rio, janitor)
...
active_participants <- import("SummaryParticipation - Sheet1.csv") %>%
  ...
  mutate(institution_type = case_when(
    ...
    grepl("Moh", institution) ~ "ministry",
    grepl("NTP", institution) ~ "ministry",  # National TB Programme - is it Minister or Public Health institute?
    grepl("National TB Control Program", institution) ~ "ministry",
    ...
    grepl("WHO", institution) ~ "multilateral",
    TRUE ~ "unknown"
  )) %>%
...
ggplot(by_date, aes(x=Date, y=n, fill=institution_type)) + geom_bar(stat = "identity")
